...
public class PhotoSelectionActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
    	...
    	mPhotoGallery.setOnItemClickListener(new OnItemClickListener() {
    		public void onItemClick(AdapterView parent, View v, int position, long id) {
    			try {
    				Uri uri = ContentUris.withAppendedId(Images.Media.EXTERNAL_CONTENT_URI, id); // 선택된 이미지를 지정하는 URI 객체 생성
    				...
    				mSelectedPhotoImage.setImageBitmap(resultPhotoBitmap);	 // 이미지뷰에 비트맵 이미지 설정
	...
	private void showParentActivity() {
		...
		if(mAlbumPhotoUri != null && resultPhotoBitmap != null){
			intent.putExtra(BasicInfo.KEY_URI_PHOTO, mAlbumPhotoUri);	 // 선택된 사진의 URI 정보를 인텐트의 부가 데이터로 전달
