package org.androidtown.multimemoapp;
...
import org.androidtown.multimemoapp.common.TitleBitmapButton;
import org.androidtown.multimemoapp.db.MemoDatabase;
...
public class MemoInsertActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.memo_insert_activity);	 // 메모 입력화면을 위한 XML 레이아웃 설정
    	...
    	mPhoto.setOnClickListener(new OnClickListener() {	// 사진 메뉴를 포함한 대화상자 띄우기
			public void onClick(View v) {
		...
		mMemoMode = intent.getStringExtra(BasicInfo.KEY_MEMO_MODE);								/* 메인 엑티비티에서 전달받은 인텐트 처리*/
	...
	public void processIntent(Intent intent) {
		...
		mMediaVideoId = intent.getStringExtra(BasicInfo.KEY_ID_VIDEO);					/* 인텐트의 부가 데이터를 변수에 할당 */
		...
		setMediaImage(mMediaPhotoId, mMediaPhotoUri, mMediaVideoId,
						mMediaVoiceId, mMediaHandwritingId);	// 사진 정보가 있으면 화면에 표시
    ...
    public void setMediaImage(String photoId, String photoUri, String videoId,
							  String voiceId, String handwritingId) {
    ...
    public void setBottomButtons() {
    	insertSaveBtn = (TitleBitmapButton)findViewById(R.id.insert_saveBtn);			// '저장' 버튼 참조
    	insertCancelBtn = (TitleBitmapButton)findViewById(R.id.insert_cancelBtn);		// '닫기' 버튼 참조
    ...
    private void saveInput() {
    	...
    	String photoFilename = insertPhoto();	// 저장된 파일 정보를 사진 테이블에 저장
    	int photoId = -1;
    	...
    	if (photoFilename != null) {
	    	// query picture id
	    	SQL = "select _ID from " + MemoDatabase.TABLE_PHOTO + " where URI = '" + photoFilename + "'";	 // 사진 테이블에 추가된 레코드의 ID 값을 확인하기 위한 SQL 정의
	    	Log.d(TAG, "SQL : " + SQL);
	    	if (MultiMemoActivity.mDatabase != null) {
	    		Cursor cursor = MultiMemoActivity.mDatabase.rawQuery(SQL);
	    		if (cursor.moveToNext()) {
	    			photoId = cursor.getInt(0);	// 조회한 레코드의 ID 값 확인
	    		}
	    		cursor.close();
	    	}
    	}
    	...
    	Intent intent = getIntent();	/* 메인 엑티비티로 		*/
    	setResult(RESULT_OK, intent);	/* 응답 인텐트 전송 후	*/
    	...
    	if (isPhotoCaptured) { // captured Bitmap
	    	try {
	    		if (mMemoMode != null && (mMemoMode.equals(BasicInfo.MODE_MODIFY)  || mMemoMode.equals(BasicInfo.MODE_VIEW))) {
    				...
    				" where _ID = '" + mMediaPhotoId + "'";	// 기존의 사진 정보를 수정하는 경우에는 테이블에서 레코드 삭제
			    	Log.d(TAG, "SQL : " + SQL);
			    	if (MultiMemoActivity.mDatabase != null) {
			    		MultiMemoActivity.mDatabase.execSQL(SQL);
			    	}

	    			File previousFile = new File(BasicInfo.FOLDER_PHOTO + mMediaPhotoUri);	 // 기존의 사진 정보를 수정하는 경우에는 사진 파일 삭제
				...
				// Temporary Hash for photo file name (새로운 파일 이름 생성)
				photoName = createFilename();

				FileOutputStream outstream = new FileOutputStream(BasicInfo.FOLDER_PHOTO + photoName);
				resultPhotoBitmap.compress(CompressFormat.PNG, 100, outstream);	// 파일에 사진 이미지 저장
				outstream.close();

				if (photoName != null) {
					Log.d(TAG, "isCaptured            : " +isPhotoCaptured);

			    	// INSERT PICTURE INFO (PHOTO 테이블에 사진 정보 추가)
    ...
    public void setMediaLayout() {
    	...
    	mVideoBtn = (TitleBitmapButton)findViewById(R.id.insert_videoBtn);		// '동영상' 버튼 참조
    	mVoiceBtn = (TitleBitmapButton)findViewById(R.id.insert_voiceBtn); 	// '음성' 버튼 참조
    }
    ...
    private void setCalendar(){
    	...
    	Date curDate = new Date();		// 현재 일시 표시
    ...
    DatePickerDialog.OnDateSetListener dateSetListener =
			new DatePickerDialog.OnDateSetListener() {		// 날짜를 선택했을 때 '일시 표시' 버튼에 날짜 표시
	...
	protected Dialog onCreateDialog(int id) {
		...
		switch(id) {
			...
			case BasicInfo.CONTENT_PHOTO:
				builder = new AlertDialog.Builder(this);	// 대화상자 객체를 만들기 위한 빌더 객체 생성
				...
				builder.setSingleChoiceItems(mSelectdContentArray, 0,
                                            new DialogInterface.OnClickListener() {	// 대화상자에 '메뉴' 버튼 설정
	                public void onClick(DialogInterface dialog, int whichButton) {	// 메뉴를 선택했을 때 선택된 메뉴의 인덱스 확인
	                	mChoicedArrayItem = whichButton;
	                }
	            });
				builder.setPositiveButton(R.string.selection_btn, new DialogInterface.OnClickListener() {
	        	    public void onClick(DialogInterface dialog, int whichButton) {		// '선택' 버튼을 눌렀을 때의 이벤트 처리
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
		...
		switch(requestCode) {
			case BasicInfo.REQ_PHOTO_CAPTURE_ACTIVITY:  // 사진 찍는 경우 (사진 촬영 액티비티로부터의 응답 처리)
			...
			case BasicInfo.REQ_PHOTO_SELECTION_ACTIVITY:  // 사진을 앨범에서 선택하는 경우 (앨범 선택 액티비티로부터의 응답 처리)
				...
				if (resultCode == RESULT_OK) {
					...
					try {
						resultPhotoBitmap = BitmapFactory.decodeStream(	// URI 정보를 이용해 비트맵 이미지로 생성
								getContentResolver().openInputStream(getPhotoUri), null, null);
