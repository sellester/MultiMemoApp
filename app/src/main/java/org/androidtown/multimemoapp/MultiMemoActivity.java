...
public class MultiMemoActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // 메모 리스트
        mMemoListView = (ListView)findViewById(R.id.memoList);  // XML 레이아웃에 정의된 리스트뷰 객체 참조
        mMemoListAdapter = new MemoListAdapter(this);   // 어댑터 객체 생성
        mMemoListView.setAdapter(mMemoListAdapter);             // 리스트뷰에 어댑터 객체 설정
        mMemoListView.setOnItemClickListener(new OnItemClickListener() {  // 리스트뷰 아이템을 선택했을 때 이벤트 처리
        ...
        newMemoBtn.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                ...
                intent.putExtra(BasicInfo.KEY_MEMO_MODE, BasicInfo.MODE_INSERT);                /* 메모 입력화면 띄우기 */
    ...
    public void openDatabase() {
        ...
        mDatabase = MemoDatabase.getInstance(this);  // MemoDatabase 객체 참조
        boolean isOpen = mDatabase.open();  // open() 메소드를 호출하여 데이터베이스 열기
        if (isOpen) {
    ...
    public int loadMemoListData() {
        String SQL = "select _id, INPUT_DATE, CONTENT_TEXT, ID_PHOTO, ID_VIDEO," +
                     " ID_VOICE, ID_HANDWRITING from MEMO order by INPUT_DATE desc";    // 메모 테이블 조회를 위한 SQL 정의

        int recordCount = -1;
        if (MultiMemoActivity.mDatabase != null) {
            Cursor outCursor = MultiMemoActivity.mDatabase.rawQuery(SQL);   // rawQuery() 메소드를 이용해 SQL 실행
            ...
            for (int i = 0; i < recordCount; i++) {
                outCursor.moveToNext();  // 커서 객체의 레코드를 하나씩 확인
    ...
    private void viewMemo(int position) {   // 메인 엑티비티의 리스트뷰에 나타난 메모를 눌렀을 때 호출됨
