...
public class HandwritingMakingActivity extends Activity {
	...
	private static final String TAG = "HandwriteMakingActivity";

	HandwritingView mWritingBoard;	 // 페인트보드 변수 선언
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.handwriting_making_activity);	// XML 레이아웃 설정
	...
	public void setWritingBorad() {
		LinearLayout boardLayout = (LinearLayout) findViewById(R.id.boardLayout);	 // 페인트보드 객체를 추가할 레이아웃 참조
	...
	public void setTopLayout() {
        ...
        mColorBtn.setOnClickListener(new OnClickListener() {	// '색상' 버튼 클릭 시 이벤트 처리
        	public void onClick(View v) {
        		...
        		Intent intent = new Intent(getApplicationContext(), ColorPaletteDialog.class);	// '색상 선택' 대화상자 띄우기
    ...
    public void saveHandwritingMaking() {
    	...
    	try {
        	...
        	File file = new File(BasicInfo.FOLDER_HANDWRITING + handwritingName);	// 기존에 만들어진 이미지 파일이 있을 경우 삭제
        	if(file.exists()) {
        		file.delete();
        	}
			...
			mBitmap.compress(Bitmap.CompressFormat.PNG, 100, outstream);  // 손글씨 이미지 저장
			outstream.close();
